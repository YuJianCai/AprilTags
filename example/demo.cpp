...
const string usage =
    ...
    "  -W <width>      Image width (default 640, availability depends on "
    "camera)\n"
    "  -H <height>     Image height (default 480, availability depends on "
    "camera)\n"
    "  -S <size>       Tag size (square black frame) in meters\n"
    "  -E <exposure>   Manually set camera exposure (default auto; range "
    "0-10000)\n"
    "  -G <gain>       Manually set camera gain (default auto; range 0-255)\n"
    "  -B <brightness> Manually set the camera brightness (default 128; range "
    "0-255)\n"
    "\n";
 ...
 */
void wRo_to_euler(const Eigen::Matrix3d &wRo,
                  double &yaw,
                  double &pitch,
                  double &roll) {
...
class Demo {
 ...
 public:
  // default constructor
  Demo()
      :  // default settings, most can be modified through command line options
         // (see below)
        m_tagDetector(NULL),
  ...
  void parseOptions(int argc, char *argv[]) {
    int c;
    while ((c = getopt(argc, argv, ":h?adtC:F:H:S:W:E:G:B:D:p:")) != -1) {
  ...
  void processImage(cv::Mat &image, cv::Mat &image_gray) {
    ...
    // cv::resize(image_gray, image_gray, cv::Size(), 0.5, 0.5);
    // LOG(INFO) << "sample image size is: " << image_gray.size();
    ...
    LOG(INFO) << detections.size() << " tags detected:" << endl;
  ...
  void ListFilesInDir(const std::string &path,
                      const std::string &ext,
                      std::vector<std::string> *files_ptr,
                      const bool &iter_flag) {
    for (auto &file : boost::make_iterator_range(
             boost::filesystem::directory_iterator(path), {})) {
      // enter sub directory
      if (boost::filesystem::is_directory(file) && iter_flag) {
        ListFilesInDir(file.path().string(), ext, files_ptr, iter_flag);
      }
      // add file path
      if (file.path().extension() == ext) {
        files_ptr->push_back(file.path().string());
      }
    }
  }
  ...
  void loadImages() {
    ...
    ListFilesInDir(m_filepath, ".png", &image_names, false);
    ...
    for (const auto &name : image_names) {
      ...
      LOG(INFO) << "image type is:" << image.channels();
